...
EXPECTED_SHA256="6b8a640ef7d2728d92357a2778cb19fb46033a6f4a052b0ec0e748fbcce8d221"

echo "== 1/6 Verificando se cora.sqlite já foi transferido (Script 1) =="
if [ ! -f cora-api/cora.sqlite ]; then
  echo "❌ cora-api/cora.sqlite não encontrado. Rode o Script 1 (transferência) primeiro."
  exit 1
fi
...
if [ "$ACTUAL_SHA256" != "$EXPECTED_SHA256" ]; then
  ...
  echo "   Rode o Script 1 novamente antes de continuar."
  exit 1
fi
echo "✅ cora.sqlite presente e íntegro."
echo ""

echo "== 2/6 Backup de segurança do Postgres atual (antes da migração) =="
...
echo "== 3/6 Gravando versão corrigida (OPEN_READONLY) de migrate-sqlite-to-postgres.js no host =="
...
JSEOF
echo "✅ Script de migração atualizado (fix: leitura em modo OPEN_READONLY) gravado em cora-api/scripts/migrate-sqlite-to-postgres.js"
echo ""

echo "== 4/6 Copiando script atualizado e cora.sqlite para dentro do container erp-app =="
...
echo "== 5/6 Rodando a migração dentro do container (usa DATABASE_URL do ambiente do container) =="
docker exec erp-app node scripts/migrate-sqlite-to-postgres.js
echo ""

echo "== 6/6 Concluído =="
echo "Se tudo acima estiver OK (sem divergências na verificação), a migração de dados terminou."
echo "Recomenda-se reiniciar o app para garantir estado limpo:"
echo "  docker compose -f docker-compose.yml -f docker-compose.override.yml restart erp-app"
